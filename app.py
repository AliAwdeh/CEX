...
def _init_state() -> None:
    defaults = {
        ...
        # When set, the run evaluates ONLY these customer journey IDs (random sampler).
...
def _display_column_name(column: str) -> str:
    ...
    special = {
        "conversation_id": "Customer journey ID",
        "customer_journey_id": "Customer journey ID",
...
def tab_run() -> None:
    ...
    with pick_cols[0]:
        if st.button(
            ...
            use_container_width=True,
            help=(
                "Pick a random sample of customer journey IDs from the uploaded CSV. "
...
def tab_review() -> None:
    ...
    search = st.text_input(
        "Search by customer phone, customer name, source conversation ID, result, or problem summary",
        value="",
    ).strip()
    if search:
...
def tab_debug() -> None:
    ...
    if ids:
        sel = st.selectbox("Customer journey ID", ids)
