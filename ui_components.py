...
def render_conversation_summary_card(conv_result: dict) -> None:
    ...
    st.markdown(
        f"""
        <div style="border-left:6px solid {color}; padding:12px 16px; background:#111827; border:1px solid #334155; border-radius:8px; margin-bottom:10px;">
          <div style="font-size:0.9rem; color:#cbd5e1; font-weight:800;">Overall result</div>
          <div style="font-size:1.3rem; font-weight:900; color:#f8fafc;">{html.escape(classification)}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
